...
> **Adaptada do ECC:** `netmiko-ssh-automation` — via `ecc-install.sh`
> **Fonte original:** `ECC/skills/netmiko-ssh-automation/SKILL.md`

## Descrição

--- name: netmiko-ssh-automation description: Safe Python Netmiko patterns for read-only collection, bounded batch SSH, TextFSM parsing, guarded config changes, timeouts, and network automation error handling.
