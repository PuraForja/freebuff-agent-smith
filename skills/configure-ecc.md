...
> **Adaptada do ECC:** `configure-ecc` — via `ecc-install.sh`
> **Fonte original:** `ECC/skills/configure-ecc/SKILL.md`

## Descrição

--- name: configure-ecc description: Interactive installer for Everything Claude Code — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
