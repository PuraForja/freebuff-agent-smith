...
> **Adaptada do ECC:** `homelab-wireguard-vpn` — via `ecc-install.sh`
> **Fonte original:** `ECC/skills/homelab-wireguard-vpn/SKILL.md`

## Descrição

--- name: homelab-wireguard-vpn description: WireGuard VPN server setup, peer configuration, key generation, split tunneling vs full tunnel routing, and remote access to a home network from mobile and laptop clients.
