...
> **Adaptada do ECC:** `healthcare-eval-harness` — via `ecc-install.sh`
> **Fonte original:** `ECC/skills/healthcare-eval-harness/SKILL.md`

## Descrição

--- name: healthcare-eval-harness description: Patient safety evaluation harness for healthcare application deployments. Automated test suites for CDSS accuracy, PHI exposure, clinical workflow integrity, and integration compliance. Blocks deployments on safety failures.
