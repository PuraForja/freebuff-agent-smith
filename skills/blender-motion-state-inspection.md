...
> **Adaptada do ECC:** `blender-motion-state-inspection` — via `ecc-install.sh`
> **Fonte original:** `ECC/skills/blender-motion-state-inspection/SKILL.md`

## Descrição

--- name: blender-motion-state-inspection description: Use this skill when inspecting Blender characters, rigs, poses, animation retargeting, ground contact, facing direction, or model-vs-motion alignment where screenshots alone are not enough.
