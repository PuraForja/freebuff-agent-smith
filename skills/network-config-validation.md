...
> **Adaptada do ECC:** `network-config-validation` — via `ecc-install.sh`
> **Fonte original:** `ECC/skills/network-config-validation/SKILL.md`

## Descrição

--- name: network-config-validation description: Pre-deployment checks for router and switch configuration, including dangerous commands, duplicate addresses, subnet overlaps, stale references, management-plane risk, and IOS-style security hygiene.
